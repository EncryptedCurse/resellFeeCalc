...
export default function Mercari() {
	/* states + hooks */
	...
	const sellingFeeRate = feeConstants.selling;
	...
	/* side effects */
	useEffect(() => handleValueCalculation(formValues, sellingFeeRate, transactionFeeRates), [formValues]);
	...
	const FeeRates = useMemo(
		() => [
			...
			' + ',
			<BottomTooltip label="Transaction fee">
				{formatCurrency(transactionFeeRates.fixed)}
			</BottomTooltip>,
			')',
		],
		[]
